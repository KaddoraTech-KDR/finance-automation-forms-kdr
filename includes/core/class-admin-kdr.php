...
if (!class_exists('FAFKDR_Admin_kdr')):
	...
	final class FAFKDR_Admin_kdr
	{
		...
		public function enqueue_assets_kdr(string $hook): void
		{
			...
			if (empty($_GET['page']) || false === strpos(sanitize_text_field($_GET['page']), 'fafkdr')) {
				return;
			}
			...
			<div class="wrap fafkdr-admin">
				...
				<table class="widefat fixed striped">
					...
					<tbody>
						<?php if (empty($rows)): ?>
							<tr>
								<td colspan="5"><?php esc_html_e('No submissions found yet.', 'finance-automation-forms-kdr'); ?></td>
							</tr>
						<?php else: ?>
		...
		public function render_settings_kdr(): void
		{
			...
			<div class="wrap fafkdr-admin">
				...
				<div class="fafkdr-card">
					<p><?php esc_html_e('v1 settings will be added here (business details, GST defaults, email templates, payment gateway keys).', 'finance-automation-forms-kdr'); ?>
					</p>
				</div>
			</div>
			<?php
